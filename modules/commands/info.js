...
module.exports.run = async({api,event,args}) => {
    ...
    const res = await axios.get(`https://api.leanhtruong.net/v2/info_user_facebook.php?api_key=LEANHTRUONG=ouSpLbBGcWenOOSjJbWaVXaq5z3VsvQYHR4oh0oBrc7v3w5D8v=APIKEY=PLANFREE&id=${uid}`);  
    ...
    const res = await axios.get(`https://api.leanhtruong.net/v2/info_user_facebook.php?api_key=LEANHTRUONG=ouSpLbBGcWenOOSjJbWaVXaq5z3VsvQYHR4oh0oBrc7v3w5D8v=APIKEY=PLANFREE&id=${mentions}`);  
    var gender = res.data.gender == 'male' ? "Nam" : res.data.gender == 'female' ? "Nữ" : "Không công khai";
    var birthday = res.data.birthday ? `${res.data.birthday}` : "Không công khai";
    var love = res.data.user_love ? `${res.data.user_love}` : "Chưa kết hôn"
    var location = res.data.location ? `${res.data.location}` : "Không công khai"
    ...
    const res = await axios.get(`https://api.leanhtruong.net/v2/info_user_facebook.php?api_key=LEANHTRUONG=ouSpLbBGcWenOOSjJbWaVXaq5z3VsvQYHR4oh0oBrc7v3w5D8v=APIKEY=PLANFREE&id=${args[0]}`);  
    var gender = res.data.gender == 'male' ? "Nam" : res.data.gender == 'female' ? "Nữ" : "Không công khai";
    var birthday = res.data.birthday ? `${res.data.birthday}` : "Không công khai";
    var love = res.data.user_love ? `${res.data.user_love}` : "Chưa kết hôn"
    var location = res.data.location ? `${res.data.location}` : "Không công khai"
